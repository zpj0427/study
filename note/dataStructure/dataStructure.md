...
* 队列本身是有序列表，若使用数组的结构存储队列，则队列的声明如上图第一小图
...
* 拷贝原数组后：array = {1, 2, 3, 4, 5, 6, 6}，数组长度为fbl[k] - 1，此时k=5，则array长度应该为7
* 则数组的的各部分组成如下
...
* **插入**：查找插入的位置并插入；插入后进行自平衡；红黑树插入总是以红色节点进行插入，如果以黑色插入，则直接破坏黑平衡！
        ...
        private void balanceTree(Node currNode, Node parentNode) {
            ...
            if (currNode.isRed() && parentNode.isRed()) {
                ...
                if (null != uncleNode && uncleNode.isRed()) {
                    ...
                    // 此处不一定是插入节点, 可能是递归处理节点
                    // 如果当前节点是插入节点, 则叔叔节点如果存在一定为红色, 不然构不成黑平衡
